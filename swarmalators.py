"""
Copyright by Dr. Kevin P. O'Keeffe
...
class Swarmalarator(metaclass=ABCMeta):
    def __init__(self, a, dt, T, n, L):
        super(Swarmalarator, self).__init__()
        np.random_seed(0)
        self.a = a
        self.dt= dt
        self.T = T
        self.n = n
        self.L = L
        self.x- = np.random.uniform(-self.L, self.L, self.n)
        self.theta0 = np.random.uniform(-np.pi, np.pi, self.n)
        self.omega = np.zeros(self.n)

        #J,K = 0.1, 1      #static sync state
        #J,K = 0.1, -1     #static async state
        self.J = 1.0
        self.K = 0       #static phase wave 
        #J,K = 1.0, -0.1,  #splintered phase wave --- note, long transient here
        #J,K = 1.0, -0.75  #active phase wave

        self.z0 = [x0, y0, theta0]
        self.t = [dt*i for i in range(int(T/dt))]
        self.z0 = None

    def solve(self):
        #Do simulation
        tic = time.clock()
        self.z0 = np.array([self.x0,self.y0, self.theta0])
        sols = odeint(f.rhs_unit_vector, z0, t, args=(J,K,n,omega))
        x, y, theta = f.unpack(sols,n) self.z0 = z0.flatten()
    ...
    def rhs_unit_vector(self, z,t,J,K,n,omega):
        """
        z0 = [x0, y0, theta0], where x0[i], y0[i], theta0[i] gives the x[t=0], y[y=0], theta[t=0] of the i-th position
        t = 
        J = float, parameter J
        K = float
    ...
    def rhs_linear(self, z,t,J,K,n,omega):
        #Instantiate
        x = z[0:n]
        y = z[n:2*n]
        theta = z[2*n:3*n]
    ...
    def rhs_linear_spatial_gaussian_phase(self, z,t,J,K,sigma,n,omega):
        #Instantiate
        x = z[0:n]
        y = z[n:2*n]
        theta = z[2*n:3*n]
    ...
    def rhs_linear_spatial_parabolic_phase(self, z,t,J,K,sigma,n,omega):
        #Instantiate
        x = z[0:n]
        y = z[n:2*n]
        theta = z[2*n:3*n]
    ...
    def scatter_t(self, x,y,theta,t):
    ...
    def find_vel_t(self, x,y,theta,dt):
    ...
    def find_W_plus(self, x,y,theta):
    ...
    def find_W_minus(self, x,y,theta):
        """ Finds time series of order parameter W^+  = < e^{i*( phi_j - theta_j)} >_j -- see paper """
        numT, num_osc = x.shape
    ...
    def find_X_minus(self, index,x,y,theta):
        numT, num_osc = x.shape
        X = 1j*np.zeros(numT)
        for t in range(numT):
            temp = 0.0+0*1j
    ...
    def find_X_plus(self, index,x,y,theta):
        numT, num_osc = x.shape
        X = 1j*np.zeros(numT)
        for t in range(numT):
            temp = 0.0+0*1j
    ...
    def find_Y_minus(self, index,x,y,theta):
        numT, num_osc = x.shape
        Y = 1j*np.zeros(numT)
        for t in range(numT):
            temp = 0.0+0*1j
    ...
    def find_Y_plus(self, index,x,y,theta):
        numT, num_osc = x.shape
        Y = 1j*np.zeros(numT)
        for t in range(numT):
            temp = 0.0+0*1j
    ...
    def find_Z(self, index,x,y,theta):
        numT, num_osc = x.shape
        Z = 1j*np.zeros(numT)
        for t in range(numT):
            temp = 0.0+0*1j
    ...
    def find_X_minus_final(self,x,y,theta):
        numT, num_osc = x.shape
    ...
    def find_X_plus_final(self, x,y,theta):
        numT, num_osc = x.shape
        X = 1j*np.zeros(num_osc)
        t = -1
        for i in range(num_osc):
    ...
    def find_Y_minus_final(self, x,y,theta):
        numT, num_osc = x.shape
    ...
    def find_Y_plus_final(self, x,y,theta):
        numT, num_osc = x.shape
        Y = 1j*np.zeros(num_osc)
        t = -1
        for i in range(num_osc):
    ...
    def find_Z_final(self, x,y,theta):
        numT, num_osc = x.shape
    ...
    def unpack_timestep(self, sols,n):
    ...
    def find_W_plus_timestep(self,x,y,theta):
    ...
    def find_W_minus_timestep(self, x,y,theta):
